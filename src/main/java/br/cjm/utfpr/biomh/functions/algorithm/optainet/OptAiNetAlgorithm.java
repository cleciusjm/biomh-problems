package br.cjm.utfpr.biomh.functions.algorithm.optainet;
...
public class OptAiNetAlgorithm implements Callable<FunctionsSolution> {
	private static final double NEIGHBORHOOD_TAX_BY_AVG = 0.3;
	...
	public OptAiNetAlgorithm(FunctionsFitnessFunction fitnessFunction, int iterations, int population, int chromosomeSize, double upperBound,
							 double lowerBound) {
	...
	public FunctionsSolution call() {
		...
		for (int i = 0; i < iterations; i++) {
			...
			List<List<Agent>> neighborhoods = separateNeighborhoods(population);

			for (List<Agent> neighborhood : neighborhoods) {
				neighborhood.stream().sorted(Comparator.comparing(Agent::fitness).reversed()).skip(1).forEach(population::remove);
			}
	...
	private List<List<Agent>> separateNeighborhoods(List<Agent> population) {
		boolean[] separated = new boolean[population.size()];
		...
		for (int i = 0; i < population.size(); i++) {
			if (!separated[i]) {
				...
				for (int j = 0; j < distances.length; j++) {
					distances[j] = current.distanceTo(population.get(j));
				}
				double mean = StatUtils.mean(distances);
				for (int j = 0; j < distances.length; j++) {
					if (!separated[j] && distances[j] < (NEIGHBORHOOD_TAX_BY_AVG * mean)) {
						neighborhood.add(population.get(j));
						separated[j] = true;
					}
				}
